...
var Botkit = {
  config: {
    ws_url: (location.protocol === 'https:' ? 'wss' : 'ws') + '://' + location.host + '/bots/botkit/api',
    webhook_url: location.origin + '/bots/botkit/api',
  ...
  request: function(url, body) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      xmlhttp.open("POST", that.config.webhook_url + url, true);
  ...
  getHistory: function() {
    that = this;
    if (that.guid) {
      that.trigger('history_loaded', JSON.parse(localStorage.getItem(that.guid)) || []);
    }
  },
  ...
  renderMessage: function(message) {
    ...
    if (message.attachments) {
      message.attachments.forEach(attachment => {
        if (attachment.text) {
          attachment.html = converter.makeHtml(attachment.text);
        }
      });
    }
    ...
    that.message_list.scrollTo(0, that.message_list.scrollHeight);
  },
  ...
  boot: function(user) {
    ...
    that.on('connected', function() {
      ...
      var history = JSON.parse(localStorage.getItem(that.guid)) || [];
      if (history.length === 0) {
        that.deliverMessage({
          type: 'message',
          text: 'Hi',
    ...
    that.on('sent', function(message) {
      ...
      localStorage.setItem(that.guid, JSON.stringify(history));
    });
